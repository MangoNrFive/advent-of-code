...
heat_loss_map = np.broadcast_to(init[..., None], init.shape+(4,))
heat_loss_map = np.broadcast_to(heat_loss_map[..., None], heat_loss_map.shape+(4,)).copy()
heat_loss_map[1, 1, 0, :] = 0

FIELD = np.broadcast_to(FIELD[..., None], FIELD.shape+(4,))
...
while True:
	...
	heat_loss_map[:-1, :, 1, 0] = np.minimum(heat_loss_map[:-1, :, 1, 0], np.min(heat_loss_map[1:, :, :, 2:], axis=(2, 3)) + FIELD[:-1, :, 1, 0])

	# south
	heat_loss_map[1:, :, 1:, 1] = np.minimum(heat_loss_map[1:, :, 1:, 1], heat_loss_map[:-1, :, :-1, 1] + FIELD[1:, :, 1:, 1])
	heat_loss_map[1:, :, 1, 1] = np.minimum(heat_loss_map[1:, :, 1, 1], np.min(heat_loss_map[:-1, :, :, 2:], axis=(2, 3)) + FIELD[1:, :, 1, 1])

	# east
	heat_loss_map[:, 1:, 1:, 2] = np.minimum(heat_loss_map[:, 1:, 1:, 2], heat_loss_map[:, :-1, :-1, 2] + FIELD[:, 1:, 1:, 2])
	heat_loss_map[:, 1:, 1, 2] = np.minimum(heat_loss_map[:, 1:, 1, 2], np.min(heat_loss_map[:, :-1, :, :2], axis=(2, 3)) + FIELD[:, 1:, 1, 2])

	# west
	heat_loss_map[:, :-1, 1:, 3] = np.minimum(heat_loss_map[:, :-1, 1:, 3], heat_loss_map[:, 1:, :-1, 3] + FIELD[:, :-1, 1:, 3])
	heat_loss_map[:, :-1, 1, 3] = np.minimum(heat_loss_map[:, :-1, 1, 3], np.min(heat_loss_map[:, 1:, :, :2], axis=(2, 3)) + FIELD[:, :-1, 1, 3])
